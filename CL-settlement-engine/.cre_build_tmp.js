...
var configSchema = exports_external.object({
  ...
  marketApiUrl: exports_external.string(),
...
function fetchOpenSeaFloorViaHTTP(runtime2, http2, slug, openSeaApiKey) {
  const response = http2.sendRequest(runtime2, (sendRequester) => {
    ...
    const data = JSON.parse(Buffer.from(res.body).toString());
    return { floor: Number(data.total?.floor_price ?? 0) };
  }, ConsensusAggregationByFields({
    floor: identical
  }))().result();
  return response.floor;
}
function fetchAlchemyFloorViaHTTP(runtime2, http2, contract, chain, alchemyApiKey) {
  const endpoints = {
...
function computeOutcome(runtime2, http2, market, keys) {
  if (market.category.toLowerCase() === "battle") {
    const finalA = fetchOpenSeaFloorViaHTTP(runtime2, http2, market.collectionA, keys.openSeaApiKey);
    const finalB = fetchOpenSeaFloorViaHTTP(runtime2, http2, market.collectionB, keys.openSeaApiKey);
    const growthA = (finalA - Number(market.initialFloorA)) / Number(market.initialFloorA) * 100;
    const growthB = (finalB - Number(market.initialFloorB)) / Number(market.initialFloorB) * 100;
    return growthA > growthB;
  }
  const floor = market.chain === "berachain" || market.chain === "monad" ? fetchOpenSeaFloorViaHTTP(runtime2, http2, market.collectionA, keys.openSeaApiKey) : fetchAlchemyFloorViaHTTP(runtime2, http2, market.collectionA, market.chain, keys.alchemyApiKey);
...
function resolveMarketOnChain(runtime2, marketId, outcome) {
  ...
  if (!network248) {
    throw new Error(`Unknown chain: ${runtime2.config.evm.chainSelectorName}`);
  }
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  const reportPayload = encodeAbiParameters(parseAbiParameters("uint256, bool"), [marketId, outcome]);
  ...
  const resp = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.evm.proxyAddress,
  ...
  if (resp.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`On-chain resolution failed for market ${marketId} — tx status: ${resp.txStatus}`);
  }
  return marketId.toString();
}
var doResolution = async (runtime2) => {
  const http2 = new ClientCapability2;
  const alchemyApiKey = runtime2.getSecret({ id: "ALCHEMY_API_KEY" }).result().value;
  const openSeaApiKey = runtime2.getSecret({ id: "OPENSEA_API_KEY" }).result().value;
  const keys = { openSeaApiKey, alchemyApiKey };
  const response = http2.sendRequest(runtime2, (sendRequester, config) => {
    const res = sendRequester.sendRequest({
      method: "GET",
      url: config.marketApiUrl
    }).result();
    const parsed = JSON.parse(Buffer.from(res.body).toString());
    const markets = (parsed.markets ?? []).map((m) => {
      const clean2 = {
        marketId: m.marketId,
        category: m.category,
        chain: m.chain
      };
      if (m.collectionA != null)
        clean2.collectionA = m.collectionA;
      if (m.collectionB != null)
        clean2.collectionB = m.collectionB;
      if (m.direction != null)
        clean2.direction = m.direction;
      if (m.targetPrice != null)
        clean2.targetPrice = m.targetPrice;
      if (m.initialFloorA != null && m.initialFloorA !== "None")
        clean2.initialFloorA = m.initialFloorA;
      if (m.initialFloorB != null && m.initialFloorB !== "None")
        clean2.initialFloorB = m.initialFloorB;
      return clean2;
    });
    return {
      markets
  ...
  for (const market of response.markets) {
    try {
      const outcome = computeOutcome(runtime2, http2, market, keys);
      const txId = resolveMarketOnChain(runtime2, BigInt(market.marketId), outcome);
...
var initWorkflow = (config) => {
  ...
  return [
    handler(cron.trigger({ schedule: config.schedule }), doResolution)
  ];
};
async function main() {
