...
function fetchOpenSeaFloorViaHTTP(runtime2, http, slug, openSeaApiKey) {
  const response = http.sendRequest(runtime2, (sendRequester) => {
...
function fetchAlchemyFloorViaHTTP(runtime2, http, contract, chain, alchemyApiKey) {
  const endpoints = {
...
function computeOutcome(runtime2, http, market, keys) {
  if (market.category.toLowerCase() === "battle") {
    const finalA = fetchOpenSeaFloorViaHTTP(runtime2, http, market.collectionA, keys.openSeaApiKey);
    const finalB = fetchOpenSeaFloorViaHTTP(runtime2, http, market.collectionB, keys.openSeaApiKey);
  ...
  const floor = market.chain === "berachain" || market.chain === "monad" ? fetchOpenSeaFloorViaHTTP(runtime2, http, market.collectionA, keys.openSeaApiKey) : fetchAlchemyFloorViaHTTP(runtime2, http, market.collectionA, market.chain, keys.alchemyApiKey);
...
var doResolution = async (runtime2) => {
  ...
  const response = http.sendRequest(runtime2, (sendRequester, config) => {
    const res = sendRequester.sendRequest({
      method: "GET",
  ...
  for (const market of response.markets) {
    try {
      const outcome = computeOutcome(runtime2, http, market, keys);
